import React from "react";

function AttendanceCard({
  monthWeeks,
  vMonthWeeks,
  totalData,
  currentWeekIndex,
}) {
  return (
    <div>
    {currentWeekIndex !== -1 && (
      <div>
        <div className="flex justify-center pb-5 space-x-5">
          {monthWeeks[currentWeekIndex].members && (
            <div className="flex items-center bg-white border rounded-lg shadow-md p-4">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 text-[#A2C579]"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0A17.933 17.933 0 0 1 12 21.75c-2.676 0-5.216-.584-7.499-1.632"
                />
              </svg>
              <p className="text-gray-800 font-medium ml-2 text-sm sm:text-base md:text-lg lg:text-xl">
                {monthWeeks[currentWeekIndex].members.length}
              </p>
            </div>
          )}

          {vMonthWeeks[currentWeekIndex].members && (
            <div className="flex items-center bg-white border rounded-lg shadow-md p-4">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 text-[#61A3BA]"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M18 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0ZM3 19.235v-.11a6.375 6.375 0 0 1 12.75 0v.109A12.318 12.318 0 0 1 9.374 21c-2.331 0-4.512-.645-6.374-1.766"
                />
              </svg>
              <p className="text-gray-800 font-medium ml-2 text-sm sm:text-base md:text-lg lg:text-xl">
                {vMonthWeeks[currentWeekIndex].members.length}
              </p>
            </div>
          )}

          {totalData[currentWeekIndex] && (
            <div className="flex items-center bg-white border rounded-lg shadow-md p-4">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
                strokeWidth={1.5}
                stroke="currentColor"
                className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 text-[#D2DE32]"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z"
                />
              </svg>
              <p className="text-gray-800 font-medium ml-2 text-sm sm:text-base md:text-lg lg:text-xl">
                {totalData[currentWeekIndex]}
              </p>
            </div>
          )}
        </div>
      </div>
    )}
  </div>


  );
}

export default AttendanceCard;
